...
export default function Home() {
  ...
  const decrypt = async() => {
    const a = await lit.decrypt(ciphertext, dataToEncryptHash, accessControlConditions);
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      <div>
        ...
        <div>
          ...
          <pre className="text-wrap break-words max-w-screen-md">{ciphertext}</pre>
        </div>
        <div>
          <label>Data to encrypt hash:</label>
          <pre className="text-wrap break-words max-w-screen-md">{dataToEncryptHash}</pre>
        </div>
