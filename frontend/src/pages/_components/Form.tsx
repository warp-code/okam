...
import { Category, Dataset, FormModel } from "@/app/types";
...
export default function Form({
  ...
  createDatasetFunction: Function;
  ...
  const createDataset = async (model: FormModel) => {
    ...
    const { data, error } = await createDatasetFunction(dataset);
          ...
          >
            {(field) => (
              <ImageUploader
                ...
                name={field.name}
                value={field.state.value}
                handleOnChange={async (file: File) => {
